...
class DataLoader:
    ...
    def load_subjects(self, subset: str) -> np.ndarray:
        """Load per-sample subject IDs for group-aware splitting."""
        file_path = self.config.dataset_dir / subset / f"subject_{subset}.txt"
        try:
...
class DataPreprocessor:
    ...
    def subject_aware_split(
        ...
        """
        Split samples so that every subject appears in exactly one of train/val.

        This prevents the same person's windows from leaking across the split,
        which is critical for UCI HAR where rows are grouped by subject.
        """
